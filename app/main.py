...
def generate_user_id():
    return str(uuid.uuid4())  # 고유한 사용자 ID 생성
...
async def calculate_result(request: Request, user_id: str, db: Session = Depends(get_db)):
    ...
    if characters:
        ...
        for character in characters:
            ...
            character_mbti = db.query(models.CharacterMBTI).filter(models.CharacterMBTI.character_id == character.id).first()
            # MBTI 특성별 점수 계산
            match_score = 0
        ...
        if best_character:
            ...
            character_image_url = f"/static/character_images/{best_character.image_url}"
        else:
